...
namespace Compiler.SyntacticAnalysis
{
    ...
    public class Parser
    {
        ...
        private ICommandNode ParseLoopCommand()
        {
            ...
            return new LoopCommandNode(expression, thenCommand, elseCommand, startPosition);
        }
        ...
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        private ICommandNode QuestionMarkCommand()
        {
            Debugger.Write("Parsing ? Command");
            Accept(QuestionMark);
            IExpressionNode expression = ParseExpression();
            Accept(This);
            ICommandNode command = ParseSingleCommand();
            return new QMCommandNode(expression, command);
        }
